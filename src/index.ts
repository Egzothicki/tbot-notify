...
export type OutageKind =
  | "credits"      // OpenRouter 402: prepaid balance too low for this model
  | "key_limit"    // OpenRouter 403: per-key weekly/monthly spend limit exhausted
  | "auth"         // 401: bad/rotated key
  | "rate_limit"   // 429
  | "upstream"     // 5xx from the gateway or provider
  | "timeout"      // client-side abort (AbortController) / socket timeout
  | "other";
...
export interface ReportAiOutageOptions {
  ...
  error: unknown;
  context?: { symbol?: string; source?: "scan" | "manual"; extra?: string };
...
const DEFAULT_MAIN_KINDS: OutageKind[] = ["credits", "key_limit", "auth", "rate_limit", "upstream", "other"];
...
export function errorText(err: unknown): string {
  ...
  if (err && typeof err === "object") {
    ...
    if (typeof bodyMsg === "string" && !msg.includes(bodyMsg)) msg = `${msg} | ${bodyMsg}`;
  } else {
    msg = String(err ?? "");
  }
  msg = msg.trim();
  if (msg.startsWith(AI_API_FAILURE_PREFIX)) msg = msg.slice(AI_API_FAILURE_PREFIX.length).trim();
...
export function classifyAiError(err: unknown): Classified {
  ...
  if (isTimeout && status === undefined) return { kind: "timeout", excerpt };
  ...
  if (status === 401) return { kind: "auth", status, excerpt };
  if (status === 429 || lower.includes("rate limit")) return { kind: "rate_limit", status: status ?? 429, excerpt };
  if (status !== undefined && status >= 500) return { kind: "upstream", status, excerpt };
...
export function formatTestingMessage(botName: string, c: Classified, ctx?: ReportAiOutageOptions["context"]): string {
  ...
  else if (ctx?.source) lines.push(`Source: ${ctx.source}`);
...
const lastMainNoticeAt = new Map<string, number>();
...
export async function reportAiOutage(opts: ReportAiOutageOptions): Promise<ReportAiOutageResult> {
  const now = opts.now ?? Date.now;
  const log = opts.log ?? ((m: string) => console.error(m));
  const classified = classifyAiError(opts.error);
  const result: ReportAiOutageResult = { classified, postedMain: false, postedTesting: false };
